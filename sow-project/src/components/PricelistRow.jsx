      ...
      <div className="top-bar">
        ...
        <div className="action-buttons">
          ...
          <button className="action-button" onClick={toggleAdvancedMode}>
            ...
            <img src={isAdvanced ? modeOnBtn : modeOffBtn} alt="Mode Switch" className="button-icon" />
          </button>
        </div>
      </div>
      ...
      <div className="table-wrapper">
        <div className="table">
          <div
            ...
            style={{
              gridTemplateColumns: `30px ${showArticle ? '100px' : ''} 260px ${showInPrice ? '75px' : ''} 70px ${showUnit ? '71px' : ''} ${showStock ? '75px' : ''} ${showDescription ? '270px' : ''} 30px`.replace(/\s+/g, ' ').trim(),
            }}
          >
            ...
            {showDescription && <div className="col-description">Description</div>}
            <div className="col-menu"></div>
          </div>
          
          {products.map((product, index) => (
            <div className="table-row" key={index} style={{
              gridTemplateColumns: `30px ${showArticle ? '100px' : ''} 260px ${showInPrice ? '70px' : ''} 70px ${showUnit ? '71px' : ''} ${showStock ? '75px' : ''} ${showDescription ? '270px' : ''} 30px`.replace(/\s+/g, ' ').trim()
            }}>
              <div
                className="col-arrow arrow"
                onClick={() => handleSave(index)}
                style={{ visibility: editedRows[index] ? "visible" : "hidden", cursor: "pointer" }}
              >
                →
              </div>
              ...
              {showArticle &&
                <div className="col-article">
                  <span className="bubble" contentEditable suppressContentEditableWarning
                    onBlur={(e) => handleFieldChange(index, "articleNo", e.target.innerText)}>
                    {product.articleNo}
                  </span>
                </div>
              }
              ...
              {showInPrice && (
                <div className="col-in-price">
                  <span className="bubble" contentEditable suppressContentEditableWarning
                    onBlur={(e) => handleFieldChange(index, "inPrice", e.target.innerText)}>
                    {product.inPrice}
                  </span>
                </div>
              )}
              ...
              <div className="col-price">
                <span className="bubble" contentEditable suppressContentEditableWarning
                  onBlur={(e) => handleFieldChange(index, "price", e.target.innerText)}>
                  {product.price}
                </span>
              </div>
              ...
              {showUnit && (
                <div className="col-unit">
                  <span className="bubble" contentEditable suppressContentEditableWarning
                    onBlur={(e) => handleFieldChange(index, "unit", e.target.innerText)}>
                    {product.unit}
                  </span>
                </div>
              )}
              ...
              {showDescription && (
                <div className="col-description">
                  <span className="bubble decs" contentEditable suppressContentEditableWarning
